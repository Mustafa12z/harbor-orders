-- Baseline schema captured from the services' CREATE TABLE IF NOT EXISTS migrate()
-- helpers. Idempotent expand-only; safe to re-run. App-side migrate() remains for
-- now; this PreSync Job establishes the ordered migration path going forward.
-- Rollback of a migration is a separate expand/contract change, never DROP here.
