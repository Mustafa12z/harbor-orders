...
variable "zone" {
  type        = string
  description = "Unused for regional Autopilot (cluster uses var.region). Kept for outputs / future zonal overrides."
  ...
  default = [
    {
      ...
      display_name = "dev-open-replace-with-your-ip"
    }
  ]
...
variable "grafana_otlp_endpoint" {
  type        = string
  description = "Grafana Cloud OTLP endpoint (e.g. https://otlp-gateway-prod-eu-west-2.grafana.net/otlp). Empty skips Grafana secret."
  default     = ""
  sensitive   = true
}
...
variable "name_suffix" {
  type        = string
  description = "Appended to project-scoped resource names (GSAs, Pub/Sub topics, secrets, VPC) so environments can share one GCP project. Empty for dev, which keeps existing names."
  default     = ""
}

variable "manage_project_singletons" {
  type        = bool
  description = "Whether this root manages project-wide IAM for the shared Artifact Registry (GKE node SA reader). The repository itself is created out of band via gcloud/console. Exactly one root per GCP project should set this to true."
  default     = true
}
