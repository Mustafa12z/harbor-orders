...
func main() {
	...
	otelShutdown, err := initOTel(context.Background(), "order-service")
	if err != nil {
	...
	server := &http.Server{
		Addr:         ":" + port,
		Handler:      withOTelHTTP(mux, "order-service"),
...
func listOrders(w http.ResponseWriter, r *http.Request) {
	...
	// Admin dashboard uses admin@platform.local and must see all orders.
	if customerID != "" && customerID != "admin@platform.local" {
...
func handleUpdateStatus(w http.ResponseWriter, r *http.Request) {
	...
	// Get current status + customer for downstream events
	var currentStatus, customerID, notes string
	err := db.QueryRow(
		"SELECT status, customer_id, notes FROM orders WHERE id = $1",
		req.OrderID,
	).Scan(&currentStatus, &customerID, &notes)
	if err != nil {
		httpError(w, "order not found", http.StatusNotFound)
		return
	}
