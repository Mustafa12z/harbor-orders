...
func main() {
	...
	otelShutdown, err := initOTel(context.Background(), "api-gateway")
	if err != nil {
	...
	server := &http.Server{
		Addr:         ":" + port,
		Handler:      withOTelHTTP(mux, "api-gateway"),
...
func handleProxy(w http.ResponseWriter, r *http.Request) {
	...
	// Auth check (skip for public endpoints / catalog GET)
...
// isPublicRequest allows unauthenticated catalog reads while keeping mutations protected.
func isPublicRequest(r *http.Request) bool {
	if isPublicPath(r.URL.Path) {
		return true
	}
	if r.Method == http.MethodGet {
		if r.URL.Path == "/api/inventory/products" || strings.HasPrefix(r.URL.Path, "/api/inventory/products/") {
			return true
		}
	}
	return false
