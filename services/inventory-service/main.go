...
func seedProducts() {
	...
	products := []struct {
		...
		{"mug-amber", "Amber Field Mug", "MUG-AMB-01", 18.00, 80},
		{"tote-charcoal", "Charcoal Market Tote", "TOTE-CHR-01", 24.00, 60},
		{"lamp-desk", "Brass Desk Lamp", "LAMP-DSK-01", 64.00, 40},
		{"notebook-linen", "Linen Notebook Set", "NOTE-LIN-01", 16.50, 120},
		{"candle-cedar", "Cedar Room Candle", "CNDL-CDR-01", 22.00, 90},
		{"thermos-slate", "Slate Travel Flask", "FLSK-SLT-01", 32.00, 55},
		{"apron-canvas", "Canvas Studio Apron", "APRN-CNV-01", 38.00, 45},
		{"tray-oak", "Oak Serving Tray", "TRAY-OAK-01", 48.00, 35},
	}
	...
	for _, p := range products {
		if _, err := db.Exec(
			...
			log.Printf("seed product %s: %v", p.id, err)
		}
	}
	log.Printf("Seeded %d demo products", len(products))
}
